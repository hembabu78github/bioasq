...
def retry_after_seconds(exc: Exception) -> float | None:
    """
    Parse Groq retry delays without confusing milliseconds with minutes.
    ...
    match = re.search(
        r"(?:please\s+)?try again in\s*"
        r"((?:\d+(?:\.\d+)?\s*"
        r"(?:milliseconds?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h)"
        r"\s*)+)",
        text,
        flags=re.IGNORECASE,
    )
    if match:
        ...
        total_seconds = 0.0
        for value_text, unit_text in parts:
