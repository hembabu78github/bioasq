...
def main() -> int:
    ...
    for index, job in enumerate(jobs, start=1):
        print(
            ...
            f"{job['stage7f_id']} {job['conditions']}"
        )
        completed_path = (
    ...
    jsonl_path = (
        BATCH_OUTPUT_DIR
        / f"stage7f_test_batch_{args.batch_index:02d}_results.jsonl"
    )
    csv_path = (
        BATCH_OUTPUT_DIR
        / f"stage7f_test_batch_{args.batch_index:02d}_results.csv"
    )
    summary_path = (
        BATCH_OUTPUT_DIR
        / f"stage7f_test_batch_{args.batch_index:02d}_summary.json"
    )
